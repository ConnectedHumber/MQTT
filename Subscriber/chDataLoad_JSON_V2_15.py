...
# 2.14	moved logFile definition to settings.py
#		fixed typo
# 2.15	now using dateutil parser to parse the timestamps
#	added global statements for mydb and logging to all methods to disambiguate variables
#
#----------------------------------------------------------------------
...
def on_connect(mqttc, obj, flags, rc):
	global brokerConnected,logging
...
def process_msg(msg_num,msg):
	...
	if 'timestamp' in payloadJson:
		...
		try:
			# strip off the +0000
			#plus=dateTimeString.find("+")
			# create a Python datetime object from the dateTimeString
			# workaround for python whinging about %z being a bad directive
			#recordedOnObject = datetime.datetime.strptime(dateTimeString[:plus], '%a %b %d %Y %H:%M:%S %Z')
	...
	GPS=False
	if LATITUDE in parameters.keys() or LONGITUDE in parameters.keys():
