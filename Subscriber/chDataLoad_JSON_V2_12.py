...
def on_connect(mqttc, obj, flags, rc):
	if rc==0:
		logging.info("on_connect callback ok, subscribing to Topic: %s",topicToSubscribe)
		mqttc.subscribe(topicToSubscribe, 0)
	else:
		logging.info("On connect callback error rc=%s",str(rc))
...
def process_msg(msg_num,msg):
	...
	if 'timestamp' in payloadJson:
		...
		logging.info("processMsg(%s): recorded on=%s",str(msg_num),recordedOnString)
...
#
#############################################################################
logging.info("main(): creating mqtt client")
mqttc = paho.Client()
...
logging.info("main(): Client connecting to MQTT broker")
...
try:
	...
	errmsg="main() Error connecting to the database errmsg=%s"
