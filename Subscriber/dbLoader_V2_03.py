...
# Other valid keys are listed in the reading_value_types database table.
#
...
pid_file = open("/run/dbLoader/dbLoader.pid", "w")
...
def updateLastSeen(msg_num,device_id,readings_id):
	...
	try:
		...
		logging.error("updateLastSeen(%s): %s",msg_num,e)
...
while True:
	if not brokerConnected:
		...
		if not connectToBroker():
			logging.info("main: unable to connect to broker")
			mqttc.loop_stop()
